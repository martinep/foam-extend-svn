//  ICE Revision: $Id: ValueExpressionDriver.C 1846 2010-10-26 19:53:12Z bgschaid $
...
Foam::surfaceVectorField *ValueExpressionDriver::makeFaceProjectionField()
{
    ...
    forAll(mesh_.boundaryMesh(),patchI)
    {
        ...
        forAll(fFace,faceI)
        {
            ...
            forAll(cProp,cJ)
            {
                const Foam::label patchID = mesh_.boundaryMesh().whichPatch(
                    cProp[cJ]);
                    ...
                    forAll(fProp,pointI)
                    {
                        forAll(mesh_.points()[0],compI)
                        {
                            if(
                                ...
                                fmin.component(compI) =
                                    mesh_.points()[fProp[pointI]].component(
                                        compI);
                            }
                            if(
                                mesh_.points()[fProp[pointI]].component(compI)
...
Foam::volScalarField *ValueExpressionDriver::makeRDistanceField(
    const Foam::volVectorField& r)
{
    Foam::dimensionSet nullDim(0,0,0,0,0);
    Foam::volScalarField *f=new Foam::volScalarField(
        Foam::IOobject
...
Foam::volScalarField *ValueExpressionDriver::makeScalarField(
...
Foam::volScalarField *ValueExpressionDriver::makeCellSetField(
...
Foam::volScalarField *ValueExpressionDriver::makeCellZoneField(
    const Foam::string &name)
{
    Foam::volScalarField *f=makeScalarField(0);
