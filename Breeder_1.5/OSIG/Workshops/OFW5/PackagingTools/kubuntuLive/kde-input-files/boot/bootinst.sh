...
clear
echo "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
echo "                     Welcome to (the) boot installer"
echo "            derived from SLAX (www.slax.org) created by Tomas M."
echo "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
echo
...
if [ "$MBR" != "$TARGET" ]; then
   ...
   echo "anymore. This will wipe out any existing boot loader. Be careful!"
fi
echo
...
./boot/syslinux/syslinux -d syslinux $TARGET
