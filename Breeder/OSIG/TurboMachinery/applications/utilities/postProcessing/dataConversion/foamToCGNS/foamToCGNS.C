...
#include "incompressible/simpleFoam/createFields.H"
		if ( isType<turbulenceModels::kEpsilon,turbulenceModel>( turbulence() ) )
		{
			Info << "turbulence model = " << turbulence->type() << endl;
			/*
			const turbulenceModels::kEpsilon& tm_ke = 
				dynamicCast<turbulenceModels::kEpsilon,turbulenceModel>( turbulence() );
			*/	
			const dictionary& turbulenceModelCoefs = turbulence->subDict( "kEpsilonCoeffs" );
			dimensionedScalar Cmu( turbulenceModelCoefs.lookup( "Cmu" ) );
			Info << "Cmu = " << Cmu.value() << endl;
			dimensionedScalar C1( turbulenceModelCoefs.lookup( "C1" ) );
			Info << "C1 = " << C1.value() << endl;
			dimensionedScalar C2( turbulenceModelCoefs.lookup( "C2" ) );
			Info << "C2 = " << C2.value() << endl;
		}
		else
		{
			Info << "Unknown turbulence model '" << turbulence->type() << "'\n";
		}
	}
