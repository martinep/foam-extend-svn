...
Foam::cellShape ConnectivityMapper::buildOneCellFromCGNSConnectivity( 
		vector<int>::const_iterator  connectivity_ptr,
		...
		int                          zoneIndex )
{
     ...
     {
         Warning << "unstructured mesh: detected a degenerate cell" << endl;
     }
     */
     return c;
}
		...
		list<Foam::cellShape>&  l_cells )
{
    ...
    for ( vector<int>::const_iterator p  = connectivity.begin() ; 
                                      p != connectivity.end() ;
				       )
    {
...
void ConnectivityMapper::addStructuredZone( int nnodei, int nnodej, int nnodek, const list<Foam::point>& plist )
{
	...
	{
		FatalError << "Invalid point list for a structured zone" << endl
			   << "Expecting " << (unsigned int)nnodes << " points"
			<< " but got " << (unsigned int)plist.size() << endl
			<< exit(FatalError);
	}
...
void ConnectivityMapper::addUnstructuredZone( int nn, int nc, 
					      const list<Foam::point>&     plist,
					      const vector<string>&        sectionnames, 
					      const vector<ElementType_t>& etypes, 
					      const vector< vector<int> >& connectivities )
{
	int last_index = zdata.size();
	int offset = (last_index==0)?0:(zdata[last_index-1].offset+zdata[last_index-1].n_nodes);
...
void ConnectivityMapper::addStructuredConnectivity( int indexZone, int nnodei, int nnodej, int nnodek )
{
	...
	for( int i=1 ; i<=ncelli ; i++ )
	{
		...
		{
			Warning << "Degenerated cell detected in structured mesh at (" 
			        << "i=" << nnodei+1 << ","
			        << "j=" << nnodej+1 << ","
			        << "k=" << nnodek+1 << ") "
			        << "in zone " << indexZone+1 << endl;
		}
		else
		{
...
void ConnectivityMapper::merge( double mergeTolerance )
{
	...
	int nptstotal = 0;
	for ( vector<zone_data_t>::const_iterator   z  = zdata.begin() ;
						    z != zdata.end() ;
						    z++ )
	{
		nptstotal += (*z).n_nodes;
