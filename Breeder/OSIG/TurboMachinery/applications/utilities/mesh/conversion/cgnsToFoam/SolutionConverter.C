	...
	) : cellmesh(mesh), qConverter(qc)
{
	...
	for( int indexZone=1 ; indexZone<nbZones ; indexZone++ )
	{
		...
		{
			Warning << "The set of solution fields available is not the same in all zones" << endl;
			/*
	...
	)
{
	...
	while( solutionSet.size()>0 )
	{
		...
		Quantity_t q  = soliter->quantity();
		GridLocation_t solutionlocation = soliter->location();
		...
		if ( q == PRESSURE ) scale_factor /= rho;

		// Find out what is the equivalent of this in Foam
		string foamname = fq.getFoamName( q );
		if ( foamname == "" ) // q is not a "known" Foam quantities
		{
			if ( q == CGNSOO::USER_DATA )
				Warning << "Skipping unknown CGNS scalar : " 
					<< soliter->name() 
					<< endl;
			else
				Warning << "Skipping CGNS scalar : " 
		...
		//Info << "Processing field named " << foamname << endl;
		vector<Quantity_t> required_quantities = fq.getQuantitiesInGroup(q);
		...
		vector<Foam::scalar>* globalfielddata = new vector<Foam::scalar>[ ncomponents ];
		int icomponent = 0;
		
		for( vector<Quantity_t>::const_iterator i  = required_quantities.begin();
							...
							i++, icomponent++ )
		{
			...
			case CGNSOO::CellCenter:
				...
				break;
			default:
				Warning << "Unsupported GridLocation_t found in solution" << endl;
				break;
			}
			...
			for ( int zid=0 ; zid<nbZones ; zid++ )
			{
				...
				case CGNSOO::Vertex:
					...
					break;
				case CGNSOO::CellCenter:
					Warning << "Output of solution at cell center ... untested code..." << endl;
					...
					break;
				default:
					Warning << "Unsupported GridLocation_t found in solution\n";
					break;
				}
			}
		}
		...
		if ( fq.isScalarQuantity( q ) ) // && viq.size()==1
		{
			...
			Info << "Writing scalar " << foamname << endl;
		...
		else if ( fq.isVectorQuantity( q ) )
		{
			...
			Info << "Writing vector " << foamname << endl;
			int nvals = globalfielddata[0].size();
			vector<Foam::vector> globalvfielddata( nvals );
	...
	)
{
	...
	for ( int isol=0 ; isol<nbsolutions ; isol++ )
	{
		...
		FlowSolution_t flowsolution = zone.readFlowSolution( isol, solname, gridloc );
		if ( solutionName=="" || solname==solutionName )
		{
			...
			for ( int ifield=0 ; ifield<nbfields ; ifield++ )
			{
				DataType_t  datatype;
				string      fieldname;
				DataArray_t field = flowsolution.readField( ifield, fieldname, datatype );
				Quantity_t  quantity_enum = qConverter.getEnum( fieldname );
	...
	)
{
	...
	bool failed = false;
	for ( vector<Quantity_t>::const_iterator 
