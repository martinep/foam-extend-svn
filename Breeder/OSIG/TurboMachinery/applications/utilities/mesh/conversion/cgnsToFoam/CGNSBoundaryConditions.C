...
CGNSBoundaryConditions::CGNSBoundaryConditions( const ConnectivityMapper& m, 
						const map<string,CGNSOO::BCType_t>& fammap,
						bool debug ) : 
...
static vector<foamNodeIndex> connectivityToPointList( const vector<int>& connec )
{
	set<foamNodeIndex> s;
	for ( vector<int>::const_iterator i=connec.begin() ; i!=connec.end() ; i++ )
		s.insert( static_cast<foamNodeIndex>(*i) );
	vector<foamNodeIndex> v(s.size());
	int k = 0;
	for ( set<foamNodeIndex>::const_iterator i=s.begin() ; i!=s.end() ; i++, k++ )
		v[k] = *i;
	return v;
}
...
void CGNSBoundaryConditions::addPatchesFromElements( CGNSOO::BCType_t bctype  )
{
	...
	for ( size_t i=0 ; i<sections.size() ; i++ )
	{
		...
		ElementType_t etype = sections[i].etype;
		...
		vector<int> connec = sections[i].connec;
		int expected_faces = connec.size() / CGNSElementType::getNumberOfNodesPerElement(etype);
		
		vector<foamNodeIndex> bc_point_list = connectivityToPointList( connec );
		
		// Convert CGNS indices to Foam indices using the zone mapper
		int k = 0;
		vector<foamNodeIndex> foam_point_list( bc_point_list.size() );
		for ( vector<int>::iterator i  = bc_point_list.begin() ;
					    i != bc_point_list.end() ;
					    i++ )
		{
		...
		// Remember the BC name in a set, so we can figure out how many DISTINCT one there are
		//Info << "Defining a new patch of " << foam_point_list.size() 
...
void CGNSBoundaryConditions::addBoundaryPatch( Base_t     base, 
					       int        indexZone, 
					       ZoneType_t zonetype, 
					       BC_t       bc, 
					       string     bcname, 
					       ...
					       PointSetType_t psettype )
{
	...
	if ( bctype == FamilySpecified )
	{
		...
		map<string,BCType_t>::const_iterator ifam = familymap.find( familyName );
		if ( ifam == familymap.end() )
		{
			Warning << "Family " << familyName 
			        << " was referenced in zone " << indexZone 
				<< " but has no BC associated with it" << endl;
			bc_name_ok = false;
		}
		else
		{
			bctype = ifam->second;
			if ( debug_ )
			   Info << "BC named " << bcname 
				<< " is family specified." << endl
				<< "It is mapped to new name " << familyName 
				<< " with bctype = " << bctype << endl;
			bcname = familyName;
		}
	}
	...
	else if ( bctype == BCTypeNull )
	{
		Warning << "The BC " << bcname 
	...
	string fbcname = FoamReplaceChar(bcname,' ','_');

	// vector of points indices (CGNS style, >=1) defining this patch
	vector<int> bc_point_list;
	...
	GridLocation_t gloc = CGNSOO::Vertex; 			  
	try
	{
	...
	case CGNSOO::Vertex:
		break; //ok
	case FaceCenter:
		FatalError << "BC defined by a GridLocation=FaceCenter"
			   << " - not yet supported"
			   << exit(FatalError);
		break;
        case IFaceCenter:
		FatalError << "BC defined by a GridLocation=IFaceCenter"
			   << " - not yet supported"
			   << exit(FatalError);
	case JFaceCenter:
		FatalError << "BC defined by a GridLocation=JFaceCenter"
			   << " - not yet supported"
			   << exit(FatalError);
	case KFaceCenter: 
		FatalError << "BC defined by a GridLocation=KFaceCenter"
			   << " - not yet supported"
			   << exit(FatalError);
	...
	case CellCenter:
	case EdgeCenter:
		FatalError << "BC defined by an invalid GridLocation"
			   << " - not yet supported"
			   << exit(FatalError);
	}
	...
	case CGNSOO::PointRange:
		...
		case CGNSOO::Structured:
			...
			{
				FatalError << "Invalid BC " << bcname << endl
					<< "range min[" << i_min << ":" << j_min << ":" << k_min << "]" << endl
					<< "range max[" << i_max << ":" << j_max << ":" << k_max << "]" << endl
					<< exit( FatalError );
			}
			else
			{
				...
				if ( debug_ )
					...
					<< ", defined with a range" << endl
					<< "\tBC range min: " << i_min << " : " << j_min << " : " << k_min << endl
					<< "\tBC range max: " << i_max << " : " << j_max << " : " << k_max << endl
					<< "\texpecting " << expected_faces << " faces" << endl
					<< "\tnbindices = " << nbindices << ", count = " << count << endl
					<< "zone dimensions = " << ni << "x" << nj << "x" << nk << endl;
			}
			}
			break;
		case CGNSOO::Unstructured:
			...
			else
			{
				...
				if ( debug_ )
					...
					<< " of type " << getFoamTypeName(bctype,bcname) 
					<< ", defined with a range" << endl
					<< "\tBC range min=" << i_min << ", max=" << i_max << endl
					<< "\tnbindices = " << nbindices << ", count = " << count << endl;
			}
			}
			break;
		default:
			FatalError << "Invalid zone type" 
				   << exit(FatalError);
		} // switch zonetype
		} // case PointRange
		break;
...
void CGNSBoundaryConditions::identifyBoundaryFaces()
{
    ...
    */
    ...
    if ( debug_ )
	Info << "Setup point-to-cell adressing" << endl;
    ...
    if ( debug_ )
	Info << "Lookup boundary faces" << endl;
    ...
    if ( debug_ )
	Info << "Shrinking tables" << endl;
    boundary_faces.shrink();
}

// local class behaving like a set but which also keeps track of
...
void CGNSBoundaryConditions::identifyBoundaryFaces2()
{
	...
	//forAll( faceset, face )
	if ( debug_ )
		Info << "Creating boundary face list" << endl;
...
void CGNSBoundaryConditions::pointListToFaceList( set<foamNodeIndex>& pointlist, 
						  Foam::faceList&     l_faces )
{
	...
	forAll ( boundary_faces, iface )
	{
		...
		if ( nfacevertices < 3 )
		{
			Warning << "Degenerated face : " << f << endl;
			continue; // next face
		}
...
{
	if ( debug_ )
		Info << "Identification of boundary conditions patches" << endl;
	// find out how many patches we will have
	int nbBC = 0;
	for ( set<string>::const_iterator name  = s_bcname.begin() ; 
					  name != s_bcname.end() ;
					  name++ )
	{
		if ( separatePatches )
	...
	if ( debug_ )
		Info << nbBC << " patches were identified" << endl;
	...
	if ( debug_ )
		Info << "Extracting patches" << endl;
	// Loop on all BCs, extract the points and identify associated faces
	for ( set<string>::iterator name  = s_bcname.begin() ; 
				    name != s_bcname.end() ;
				    name++ )
	{
		string bcname = *name;
		if ( debug_ )
			Info << "\tExtracting faces of patch named " << bcname << endl;
		...
		CGNSOO::BCType_t bctype = BCTypeNull;
		set<foamNodeIndex> merged_point_list;
		while( p != pend )
		{
			const patchinfo& patch = (*p).second;
			const vector<foamNodeIndex>& pl = patch.pointlist;
			merged_point_list.insert( pl.begin(), pl.end() );
			if ( bctype != BCTypeNull && bctype != patch.bctype )
			{
				Warning << "The set of patches defining the BC named " << bcname 
					<< " do not have the same BCType" << endl;
			}
			bctype = patch.bctype;
			...
			if ( p==pend || separatePatches )
			{
				...
				if ( debug_ )
					Info << "\tBC " << bcname << " (physical type : " << foam_phystype << ")"
				...
				//if( foam_phystype != "empty" )
				{
					...
					{
						if ( debug_ )
							Info << "\tNumber of faces found : " << l_faces.size() 
							<< ", expected " << nfaces
							<< endl;
		...
		)
{
	...
	//Info << "CGNSBoundaryConditions::rotationTransform: posrotT = "  << rot << endl;
...
/*	
	Info	<< "\tcomputeMatchingCyclicFaces: " << endl
		<< "\t\tfListCtrsRef            : " << fListCtrsRef[0]       << endl
		<< "\t\tfListCtrsPer            : " << fListCtrsPer[0]       << endl
		<< "\t\tfListCtrsPosTr          : " << fListCtrsPosTr[0]     << endl
		<< "\t\tfListCtrsNegTr          : " << fListCtrsNegTr[0]     << endl
		<< "\t\tminEdgeLength           : " << minEdgeLength         << endl
		<< "\t\tcyclicMatchFaceTolFactor: " << cyclicToleranceFactor << endl
		<< "\t\tmaximumMatchingDelta    : " << maximumMatchingDelta  << endl;
*/	
	...
	forAll( fListCtrsRef, iface )
	{
		...
		{
			if ( debug_ )
				Info 	<< "No match found for : " << fListCtrsRef[iface]
					<< " after trying both forward and backward tranformation" << endl
					<< "Smallest delta found : " << smallestDeltaFound << endl;
			break;
		}
	}
	...
	if( nbMatch != nbFaces )
	{
		FatalError << "Cannot find enough cyclic matches." << endl
			<< "\tFound " << nbMatch << ", expecting " << nbFaces << endl
			<< "\tCyclic match tolerance = " << maximumMatchingDelta << endl
			<< exit(FatalError);
	}
...
int CGNSBoundaryConditions::computeCyclicBC( 
	const string&       firstPatchName, 
	const string&       secondPatchName,
	...
	Foam::scalar        cyclicToleranceFactor )
{
	...
	string fpname1 = FoamReplaceChar(firstPatchName ,' ','_');
	string fpname2 = FoamReplaceChar(secondPatchName,' ','_');
	...
	{
		FatalError << "cyclic patch size mismatch." << endl
			<< "\tBC #1: " << firstPatchName  
			<< " : size=" << patchFaces[ipatch1].size() << endl
			<< "\tBC #2: " << secondPatchName 
			<< " : size=" << patchFaces[ipatch2].size() << endl
			<< exit(FatalError);
	}
	...
	if ( debug_ )
		Info << "\tPatches " << firstPatchName << " and " << secondPatchName 
...
Foam::polyMesh* CGNSBoundaryConditions::buildFoamMesh( const Foam::Time& t ) const
{
	...
	if ( debug_ )
	{
		...
		forAll( patchNames, i )
		{
			Info << "\tPatch " << i << " named " << patchNames[i] 
...
/*static*/ Foam::word CGNSBoundaryConditions::getFoamTypeName( CGNSOO::BCType_t bctype, const string& bcname )   
{
	using namespace CGNSOO;
	...
	case BCTypeUserDefined       :
		{
		string l_bcname = toLower(bcname);  // search in lowercase
	
		if( l_bcname.find("wall")    != string::npos) s_retValue = s_wall; 
		else if(l_bcname.find("in")  != string::npos) s_retValue = "inlet";
		else if(l_bcname.find("out") != string::npos) s_retValue = "outlet";
		}
		break;
...
/*static*/ bool CGNSBoundaryConditions::validateUserDefinedBCName( const string& bcname )
{
	// convert to lowercase
	string l_bcname = toLower(bcname);
	
	return	l_bcname.find("wall")    != string::npos ||
		l_bcname.find("inflow")  != string::npos ||
		l_bcname.find("outflow") != string::npos ||
		l_bcname.find("inlet")   != string::npos ||
		l_bcname.find("outlet")  != string::npos
		;
}
